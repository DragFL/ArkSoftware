...
public class Consults extends Applet {
	...
	public static Consults getInstance() {
		   if(instanciaUnica==null)
	            instanciaUnica=new Consults();
	...
	public Consults() {
		...
		JButton btnAsig = new JButton("Desasignar");
		btnAsig.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					
					controla.rutaLocalmns(plcs.getText(), munic.getText());
					System.out.println("Se desagino la ruta " + plcs.getText()+" al municipio de " +munic.getText());
					plcs.setText("");
					munic.setText("");
				} catch (Exception e1) {
					e1.printStackTrace();
		...
		btnAsig.setBounds(220, 345, 105, 29);
		add(btnAsig);
		...
		btnBuscar.setBounds(115, 345, 105, 29);
		add(btnBuscar);
	}
